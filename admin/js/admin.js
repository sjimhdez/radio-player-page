(function() {
    ...
    function removeStation(index) {
        ...
        if (row) {
            ...
            row.querySelectorAll('.radplapag-image-upload-wrapper').forEach(function(wrapper) {
                ...
                var removeBtnEl = wrapper.querySelector('.radplapag-remove-image-btn');
                if (idEl) idEl.value = '';
    ...
    function addStation() {
        ...
        if (hiddenRows.length > 0) {
            ...
            if (title) {
                ...
                title.textContent = (s.stream || 'Stream') + ' ' + visibleCount;
            }
            updateAddButton();
            updatePageOptions();
        }
    ...
    function initImageUpload() {
        ...
        container.addEventListener('click', function(e) {
            if (e.target.classList.contains('radplapag-upload-btn')) {
                ...
                var frameTitle = isProgramDefFrame ? (s.addProgramImage || 'Add program image') : (s.selectImage || 'Select Image');
                var frameButtonText = isProgramDefFrame ? (s.addProgramImage || 'Add program image') : (s.selectImage || 'Select Image');
                ...
                file_frame.on('select', function() {
                    ...
                    if (isProgramDef) {
                        ...
                        if (uploadBtn) uploadBtn.textContent = (s.changeImage || 'Change image');
                    }
                    file_frame = null;
                });
            ...
            if (e.target.classList.contains('radplapag-remove-image-btn')) {
                ...
                if (wrapper.closest('.radplapag-program-definition-row')) {
                    var uploadBtn = wrapper.querySelector('.radplapag-upload-btn');
                    if (uploadBtn) uploadBtn.textContent = (s.addProgramImage || 'Add program image');
                }
            }
        });
    }
    ...
    var stationFieldMessages = {
        ...
        stationTitleMax: s.stationTitleMax || 'Stream title must be 64 characters or less.'
    };
    ...
    function validateTimeFormat(timeString) {
        ...
        if (!timeString || !timeRegex.test(timeString)) {
            return { valid: false, message: s.invalidTimeFormat || 'Invalid time format. Times must be in HH:MM format.' };
        }
        return { valid: true };
    }
    ...
    function validateTimeRange(startTime, endTime) {
        if (!startTime || !endTime) {
            return { valid: false, message: s.completeTimeFields || 'Please complete all time fields' };
        }
        ...
        if (start === end) {
            return { valid: false, message: s.startEndSame || 'Start and end times cannot be the same' };
        }
        return { valid: true };
    }
    ...
    function checkOverlaps(dayWrapper, currentRow, excludeIndex) {
        ...
        for (var i = 0; i < allRows.length; i++) {
            ...
            if (currentStartMin < otherEndForOverlap && currentEndForOverlap > otherStartMin) {
                ...
                var otherName = (otherSel && otherSel.options[otherSel.selectedIndex]) ? otherSel.options[otherSel.selectedIndex].text : (s.unnamedProgram || 'Unnamed program');
                ...
                return {
                    valid: false,
                    message: (s.timeSlotOverlapsWith || 'This time slot overlaps with') + ': ' + formattedMessage
                };
            }
        }
        return { valid: true };
    ...
    function checkCrossDayOverlaps(programRow) {
        ...
        var dayMap = { monday: 'tuesday', tuesday: 'wednesday', wednesday: 'thursday', thursday: 'friday', friday: 'saturday', saturday: 'sunday', sunday: 'monday' };
        ...
        for (var i = 0; i < nextDayRows.length; i++) {
            ...
            if (endMin > nextStartMin) {
                ...
                showProgramError(nextRow, (s.timeSlotOverlapsWith || 'This time slot overlaps with') + ': ' + currentFormattedMessage);
                return { valid: false, message: (s.timeSlotOverlapsWith || 'This time slot overlaps with') + ': ' + nextFormattedMessage };
            }
        }
        return { valid: true };
    }
    ...
    function validateProgramRow(programRow) {
        ...
        var start = programRow.querySelector('.radplapag-program-start') ? programRow.querySelector('.radplapag-program-start').value : '';
        var end = programRow.querySelector('.radplapag-program-end') ? programRow.querySelector('.radplapag-program-end').value : '';
        ...
        if (programId) {
            ...
            if (idx >= 0 && idx < defRows.length) {
                ...
                if (programName === '') {
                    var errMsg = s.pleaseSelectProgramWithName || 'Please select a program with a name. Program name is required.';
    ...
    function checkPrevDayCrossOverlaps(programRow) {
        ...
        var currentDay = currentDayWrapper.getAttribute('data-day');
        if (!currentDay) return { valid: true };
        var prevDayMap = { monday: 'sunday', tuesday: 'monday', wednesday: 'tuesday', thursday: 'wednesday', friday: 'thursday', saturday: 'friday', sunday: 'saturday' };
        ...
        for (var i = 0; i < prevDayRows.length; i++) {
            ...
            if (prevEndMin > startMin) {
                ...
                var currentFormattedMessage = formatOverlapMessage(name, currentDay, start, end);
                showProgramError(prevRow, (s.timeSlotOverlapsWith || 'This time slot overlaps with') + ': ' + currentFormattedMessage);
                return { valid: false, message: (s.timeSlotOverlapsWith || 'This time slot overlaps with') + ': ' + prevFormattedMessage };
            }
        }
        return { valid: true };
    }
    ...
    function clearProgramError(programRow) {
        ...
        inputs.forEach(function(el) { el.classList.remove('radplapag-error'); });
        ...
        if (nextDay) {
            ...
            if (nextDayWrapper) {
                nextDayWrapper.querySelectorAll('.radplapag-program-row').forEach(function(row) {
                    if (window.getComputedStyle(row).display !== 'none') validateProgramRow(row);
                });
            }
        }
        var prevDay = prevDayMap[currentDay];
    ...
    function initScheduleManagement() {
        function sortProgramsByStartTime(programsList) {
            ...
            rows.forEach(function(row) { programsList.appendChild(row); });
        }
                ...
                if (!isStillInGroup) {
                    setTimeout(function() {
                        ...
                        if (!programRow.contains(activeElement)) {
                            ...
                            var scheduleWrapper = programRow.closest('.radplapag-schedule-wrapper');
                            if (scheduleWrapper && dayWrapper) revalidateAdjacentDays(programRow, scheduleWrapper, dayWrapper);
            ...
            container.addEventListener('click', function(e) {
                if (e.target.classList.contains('radplapag-add-program')) {
                    ...
                    newRow.innerHTML = selectHtml +
                        ...
                        '<span style="margin-right: 5px;"> ' + toLabel + ' </span>' +
                        ...
                        '<div class="radplapag-schedule-remove-cell"><a href="#" class="submitdelete radplapag-remove-program">' + removeTimeSlot + '</a></div>' +
                ...
                if (e.target.classList.contains('radplapag-remove-program')) {
                    ...
                    if (programRow) {
                        ...
                        programRow.remove();
                        if (dayWrapper) {
                            dayWrapper.querySelectorAll('.radplapag-program-row').forEach(function(row) { validateProgramRow(row); });
                        }
                        if (scheduleWrapper && dayWrapper) {
                            ...
                            if (currentDayIndex < days.length - 1) {
                                ...
                                var nextDayWrapper = scheduleWrapper.querySelector('.radplapag-schedule-day[data-day="' + nextDay + '"]');
                                if (nextDayWrapper) nextDayWrapper.querySelectorAll('.radplapag-program-row').forEach(function(row) { validateProgramRow(row); });
                            }
                            if (currentDayIndex > 0) {
                                var prevDay = days[currentDayIndex - 1];
                                var prevDayWrapper = scheduleWrapper.querySelector('.radplapag-schedule-day[data-day="' + prevDay + '"]');
                                if (prevDayWrapper) prevDayWrapper.querySelectorAll('.radplapag-program-row').forEach(function(row) { validateProgramRow(row); });
                            }
                        }
                    }
                }
    ...
    if (form && container) {
        form.addEventListener('submit', function(e) {
            ...
            container.querySelectorAll('.radplapag-station-row').forEach(function(stationRow) {
                ...
                if (!validation.valid) {
                    hasErrors = true;
                    if (!firstErrorElement && validation.firstErrorTr) firstErrorElement = validation.firstErrorTr;
                }
            });
            container.querySelectorAll('.radplapag-program-definition-row').forEach(function(defRow) {
                var stationRow = defRow.closest('.radplapag-station-row');
        ...
        container.addEventListener('click', function(e) {
            if (e.target.closest('.radplapag-schedule-toggle')) {
                ...
                if (isExpanded) {
                    ...
                    toggle.innerHTML = '<span class="toggle-indicator" aria-hidden="true"></span>' + showLabel;
                } else {
                    scheduleWrapper.classList.remove('radplapag-schedule-collapsed');
                    toggle.setAttribute('aria-expanded', 'true');
                    toggle.innerHTML = '<span class="toggle-indicator" aria-hidden="true"></span>' + hideLabel;
                }
            }
        });
    }
    ...
    function initProgramsManagement() {
        ...
        container.addEventListener('click', function(e) {
            if (e.target.classList.contains('radplapag-add-program-definition')) {
                ...
                newRow.innerHTML = '<div class="radplapag-program-definition-line">' +
                    '<div class="radplapag-program-definition-name-cell">' +
                    '<input type="text" name="radplapag_settings[stations][' + stationIndex + '][programs][' + nextIndex + '][name]" value="" placeholder="' + programNamePlaceholder + '" class="radplapag-program-definition-name" maxlength="64" style="width: 200px;">' +
                    '<div class="radplapag-program-error-message" style="display: none;"></div>' +
                    '</div>' +
                    ...
                    '<button type="button" class="button radplapag-upload-btn">' + addProgramImage + '</button>' +
                    '<button type="button" class="button radplapag-remove-image-btn" style="display:none;">' + removeImage + '</button>' +
                    '</div>' +
                    '</div>' +
                    '<div class="radplapag-program-definition-remove-cell">' +
                    '<a href="#" class="submitdelete radplapag-remove-program-definition" data-station-index="' + stationIndex + '" data-program-def-index="' + nextIndex + '">' + removeProgram + '</a>' +
                    '</div>' +
                    '</div>';
    ...
    function validateProgramDefinitionRow(defRow) {
        ...
        if (!name) {
            showProgramDefinitionError(defRow, s.programNameRequired || 'Program name is required. Enter a name to use this program in the schedule.');
            return { valid: false };
        }
        clearProgramDefinitionError(defRow);
        return { valid: true };
